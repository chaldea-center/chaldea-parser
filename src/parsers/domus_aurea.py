...
def _parse_sheet_data(key: str, gid: str, legacy: bool) -> DropRateSheet:
    ...
    df.drop(
        columns=[
            ...
            if df.iloc[0, i] != df.iloc[0, i] and df.iloc[1, i] != df.iloc[1, i]
        ],
        inplace=True,
    )
    # drop 重复的quest列
    cols = [c for c in df.columns if df.loc[1, c] == "クエスト名"]
    # print(cols)
    ...
    sheet_items = [
        _tuple for _tuple in _ITEM_MAPPING if not legacy or _tuple[-1] is not True
    ]
    assert tuple(df.iloc[1, 4:]) == tuple([_tuple[0] for _tuple in sheet_items])
    assert tuple(df.iloc[0, :3]) == ("エリア", "クエスト名", "AP") and df.iloc[0, 3] in (
        "サンプル数",
        "データ数",
    )
...
# (sheet_item, id, jp_name, TRUE(if only in new sheet))
_ITEM_MAPPING = (
    ("証", 6503, "英雄の証"),
    ("骨", 6516, "凶骨"),
    ("牙", 6512, "竜の牙"),
